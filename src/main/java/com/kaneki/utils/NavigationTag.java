...
public class NavigationTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			// 计算总页数
			int pageCount = page.getTotal() / page.getPageRows();
			if (page.getTotal() % page.getPageRows() > 0) {
				pageCount++;
			}
			...
			if (page.getPage() > 1) {
				...
				preUrl = append(preUrl, "rows", page.getPageRows());
...
writer.print("<li class=\"disabled\"><a href=\"#\">" + "上一页 </a></li>");
			...
			else if(page.getPage() - 3 <=0){
				indexPage=1;
			}else if(page.getPage() - 4 <=0){
				indexPage=1;
			}else if(page.getPage() - 5 <=0){
				indexPage=1;
			}
    ...
    for (int i= 1;i <= number && indexPage <= pageCount;indexPage++,i++){
			...
			writer.print("<li class=\"active\"><a href=\"#\">" + indexPage
				...
				pageUrl = append(pageUrl, "rows", page.getPageRows());
writer.print("<li><a href=\"" + pageUrl + "\">" + indexPage + "</a></li>");
			}
			// 显示“下一页”按钮
			if (page.getPage() < pageCount) {
				String nextUrl = append(url, "page", page.getPage() + 1);
				nextUrl = append(nextUrl, "rows", page.getPageRows());
