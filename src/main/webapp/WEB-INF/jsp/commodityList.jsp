			...
			<div class="panel panel-default">
				<div class="panel-body">
					...
					<form class="form-inline" method="post"
						action="${pageContext.request.contextPath }/commodityList.action">
						<div class="form-group">
							<label for="commodityName">商品名称：</label> <input type="text"
								class="form-control" id="commodityName" value="${commodityName}"
								name="commodityName" />
						</div>
						<div class="form-group">
							<label for="comFrom">商品类型</label> <select class="form-control"
								id="comFrom" name="comType.comTypeId">
								<option value="">--请选择--</option>
								<c:forEach items="${COMTYPE}" var="comType">
									<option value="${comType.comTypeId}" <c:if test="${comType.comTypeId == CTID}">selected</c:if> >
								    ${comType.comTypeName}
								</option>
								</c:forEach>
							</select>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
							...
							<table class="table table-bordered table-striped table-hover">
								...
								<tbody>
									<c:forEach items="${page.list}" var="com">
										<tr>
											<th>${com.commodityId}</th>
											<th>${com.comType.comTypeName}</th>
											<th>${com.commodityName}</th>
											<th>${com.placeProduction}</th>
											<th>${com.manufacturer}</th>
											<th>${com.purchasePrice}</th>
											<th>元</th>
											<th>${com.sellingPrice}</th>
											<th>元</th>
											<th>${com.volume}</th>
											<th>m³</th>
											<th>${com.minQuantity}</th>
											<th>件</th>
											<th>${com.maxQuantity}</th>
											<th>件</th>
											<td><a href="#" class="btn btn-primary btn-xs"
												data-toggle="modal" data-target="#EditDialog"
												onclick="edit(${com.commodityId})">修改</a> <a href="#"
												class="btn btn-danger btn-xs"
												onclick="deleteCommodity(${com.commodityId})">删除</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
						...
						<div class="col-md-12 text-right">
							<kaneki:page
								url="${pageContext.request.contextPath }/commodityList.action" />
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
				...
				<div class="modal-body">
					<form class="form-horizontal" id="new_form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="minQuantity" onkeyup="value=value.replace(/[^\d]/g,'')"
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="maxQuantity" onkeyup="value=value.replace(/[^\d]/g,'')"
				...
				<div class="modal-body">
					<form class="form-horizontal" id="edit_form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="edit_minQuantity" onkeyup="value=value.replace(/[^\d]/g,'')"
									placeholder="最小库存(件)" name="minQuantity" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="edit_maxQuantity" onkeyup="value=value.replace(/[^\d]/g,'')"
									placeholder="最大库存(件)" name="maxQuantity" />
							</div>
						</div>
					</form>
